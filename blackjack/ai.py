...
from game import LOSE_STATE, WIN_STATE, Game, states
...
class Agent:
    ...
    def make_one_transition(self, action):
        # state (user_sum, has_ACe, dealer_hand)
        if action == HIT:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            # # For MC values
            # self.MC_values = {} # Dictionary: Store the MC value of each state        ------> what we want
            # self.S_MC = {}      # Dictionary: Store the sum of returns in each state   -----> G (reward to go)
            # self.N_MC = {}      # Dictionary: Store the number of samples of each state -----> used for average
            # # MC_values should be equal to S_MC divided by N_MC on each state (important for passing tests)
            ...
            while not self.simulator.game_over():
                #records the number of times we see a particular state
                self.N_MC[self.simulator.state]+=1
                # store the states in order
                trajectory.append(self.simulator.state)
                #print("state",i,":", self.simulator.state)
                # advances the state
                self.make_one_transition(self.default_policy(self.simulator.state))
                i+=1
            # append the win/lose state    
            trajectory.append(self.simulator.state)
            self.N_MC[self.simulator.state]+=1
            # check if last state is win or lose
            R_n = self.simulator.check_reward()
            for state in trajectory:
                # update state value
                self.S_MC[state]+= pow(DISCOUNT,i) * R_n
                # average of MC
                count = 1 if self.N_MC[state] == 0 else self.N_MC[state]
                self.MC_values[state] = self.S_MC[state]/count
                i-=1
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                self.make_one_transition(self.default_policy(self.simulator.state))
                # update value
                self.TD_values[state] = self.TD_values[state] + alpha*(DISCOUNT*self.TD_values[self.simulator.state] - self.TD_values[state])
            #update value of terminal state as well as # seen for that state
            self.TD_values[self.simulator.state] = self.TD_values[self.simulator.state] + self.alpha(self.N_TD[self.simulator.state])*(self.simulator.check_reward() - self.TD_values[self.simulator.state])
            self.N_TD[self.simulator.state]+=1
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                # pick an action
                a = self.pick_action(self.simulator.state,epsilon)
                # alpha
                alpha = self.alpha(self.N_Q[curr_State][a])
                # transition state
                self.make_one_transition(a)
                # update Q score
                self.Q_values[curr_State][a] = self.Q_values[curr_State][a] + alpha*(DISCOUNT*max(self.Q_values[self.simulator.state][HIT], self.Q_values[self.simulator.state][STAND]) - self.Q_values[curr_State][a])
                self.N_Q[curr_State][a]+=1
            # update Q score of terminal state
            self.Q_values[self.simulator.state][a] = self.Q_values[self.simulator.state][a] + self.alpha(self.N_Q[self.simulator.state][a])*(self.simulator.check_reward() - self.Q_values[self.simulator.state][a])
            self.N_Q[self.simulator.state][a]+=1
    ...
    def pick_action(self, s, epsilon):
        ...
        if random.random() < epsilon:
            ...
            # find the best move at state: either HIT or STAND
            return HIT if self.Q_values[s][HIT] > self.Q_values[s][STAND] else STAND
